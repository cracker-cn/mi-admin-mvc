...
namespace Mi.Application.WxWork
{
    public class WxUserService : IWxUserService, IScoped
    {
        ...
        public WxUserService(WxWorkRequest request, WxConfig config, MessageModel msg, IMapper mapper)
        {
            _request = request;
            _config = config;
            _msg = msg;
        ...
        public async Task<MessageModel<IList<WxUserItem>>> GetDeptMemberListAsync(long deptId)
        {
            ...
            var result = await _request.GetAsync<WxDeptUserResponse>(url, _config.wx_work_contact_list_secret, "department_id=" + deptId);
