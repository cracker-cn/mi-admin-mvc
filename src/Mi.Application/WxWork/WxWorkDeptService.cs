...
namespace Mi.Application.WxWork
{
    public class WxWorkDeptService : IDepartmentService, IScoped
    {
        ...
        public WxWorkDeptService(WxWorkRequest request, WxConfig config, MessageModel msg)
        {
            _request = request;
            _config = config;
            _msg = msg;
        ...
        public async Task<MessageModel> AddOrUpdateDeptAsync(DepartmentOperation operation)
        {
            ...
            var result = await _request.SendAsync<WxApiResponseBase, DepartmentOperation>(url, _config.wx_work_contact_list_secret, HttpMethod.Post, operation);
        ...
        {
            var result = await _request.SendAsync<WxDept>("department/list", _config.wx_work_contact_list_secret, HttpMethod.Post);
        ...
        public async Task<MessageModel> RemoveDeptAsync(IList<long> ids)
        {
            ...
            var result = await _request.GetAsync<WxApiResponseBase>("department/delete", _config.wx_work_contact_list_secret, "id=" + id);
            return result.Succeed() ? _msg.Success() : _msg.Fail();
        }
